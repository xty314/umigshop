...
export default class HeaderBottomItem extends React.Component{
    render(){
        return(
            <li><Link to={'/products?cat='+this.props.name} replace >{this.props.name}<svg className="menu__arrow" width="6px" height="9px">
          ...
          {this.props.sub.map((ss,i)=>{
              return(
                <li> <Link to={'/products?cat='+this.props.name+"&scat="+ss.name} replace >{ss.name}</Link></li>
              )
          })}
          </ul>{/* .menu / end */}
        </div>:<div></div>}
